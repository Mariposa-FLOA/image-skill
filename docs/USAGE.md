...
2. **平台/比例（可选）**：如果有明确用途，可以写“小红书 3:4”“抖音 9:16”；如果没有指定，让 AI 先识别源图构图、方向和留白，自动选择最合适的画幅。
...
请使用 <skill-name>。
目标：<我要做的图>。
平台/比例（可选）：<平台 + 原生比例；未指定则由 AI 根据源图决定>。
必须保留：<人物/物体/文字/关系>。
禁止出现：<不想要的元素>。
先给我 test 返图，不要自动归档。
```

视觉任务不要只写“做得高级/做成某某风格”。如果有源图，智能体应先分析场景、景别、人物、物品、光线和留白，再决定分割区域、构图机制、局部色彩、文字位置和图层；如果设定卡和最终画面同时存在，要明确设定卡只负责人物身份。
...
4. 首次运行时写出 Skill 名称、平台比例（如果有）和保留项。
...
- 有明确发布用途时再写平台和原生比例；没有用途时不要强行指定，让 AI 根据源图自动选择。
