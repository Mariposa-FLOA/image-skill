...
如果希望继续在 Photoshop 里编辑，不要只上传合并后的 PNG。请让智能体在生成时保留真实的背景、主体、几何、字标、纹理和阴影图层，再使用 `poster-layered-psd-export` 导出 PSD：

```text
请把这张海报做成可在 Photoshop 继续编辑的分层 PSD。
从开始就保留真实独立图层；不要把一张 PNG 复制成多个假图层。
